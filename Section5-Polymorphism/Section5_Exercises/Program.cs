namespace Section5_Exercises
{
     ...
     * 
     * Derive two classes SqlConnection and OracleConnection from DbConnection, 
     * and provide a simple implementation of opening and closing connections using Console.WriteLine(). 
     ...
     * 
     * 
     * Exercise 2: Design a database command Now that we have the concept of a DbConnection, let’s work out how to represent a DbCommand.
     * 
     ...
     * This is polymorphism. Interestingly, DbCommand doesn’t care about how a connection is opened or closed. 
     * It’s not the responsibility of the DbCommand. All it cares about is to send an instruction to a database.
     * 
     * For running the instruction, simply output it to the Console. In the real-world, SQL Server (or any other DBMS) provides an API for running an instruction against the database.
     * In the main method, initialize a DbCommand with some string as the instruction and a SqlConnection. 
     * Execute the command and see the result on the console.Then, swap the SqlConnection with an OracleConnection and see polymorphism in action.
     */
    ...
    internal class Program
    {
        ...
        {
            var oracleCommand = new DbCommand(new OracleConnection());
            oracleCommand.Execute("add");

            var sqlCommand = new DbCommand(new SqlConnection());
            sqlCommand.Execute("add");
        }
    }
}
