 ...
 * Each activity should have a method called Execute(). The workflow engine does not care about the concrete implementation of activities. 
...
namespace Section6_Exercises
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            workflow.run(new UploadVideo());
            workflow.run(new Call());
            workflow.run(new SendEmail());
            workflow.run(new UpdateStatus());
        }
    }
